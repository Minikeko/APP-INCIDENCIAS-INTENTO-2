...
function formatEuros(importe: number | null) {
...
export default function FacturacionPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.clienteProveedor.trim()) {
      toast.error("El cliente/proveedor es obligatorio");
      return;
    }
    ...
    try {
      ...
      formData.append("clienteProveedor", form.clienteProveedor.trim());
      if (form.numeroFactura.trim()) formData.append("numeroFactura", form.numeroFactura.trim());
      if (form.importe.trim()) formData.append("importe", form.importe.trim());
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <p className="text-sm text-[var(--text-secondary)] mb-6">
        Documentos de facturación. Visible solo para administradores.
      </p>
      ...
      {loading ? (
        ...
        <p className="text-sm text-[var(--text-muted)] bg-[var(--bg-panel)] border border-[var(--border-subtle)] rounded-lg px-4 py-6 text-center">
          No hay documentos de facturación todavía.
        </p>
      ) : (
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm text-[var(--text-primary)] truncate">
                    {doc.clienteProveedor} — {MESES[doc.mes - 1]} {doc.anio}
                    {doc.numeroFactura && ` · Nº ${doc.numeroFactura}`}
                  </p>
                  <p className="text-xs text-[var(--text-muted)] truncate">
                    {formatEuros(doc.importe)} · {doc.nombreArchivo} · {formatBytes(doc.tamano)} · subido por {doc.subidoPor.nombre}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <a
                  href={`/api/facturas/${doc.id}`}
                  target="_blank"
